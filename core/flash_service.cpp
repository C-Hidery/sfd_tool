...
namespace sfd
{
    ...
    public:
        ...
        FlashStatus exportPartitionTableToXml(const std::string& output_path) override
        {
            ...
            if (!doc)
            {
                DEG_LOG(E, "exportPartitionTableToXml: xmlNewDoc failed");
                return make_error(FlashErrorCode::InternalError, "xmlNewDoc failed");
            }
            ...
            {
                xmlFreeDoc(doc);
                DEG_LOG(E, "exportPartitionTableToXml: xmlNewNode failed");
                return make_error(FlashErrorCode::InternalError, "xmlNewNode failed");
            }
            xmlDocSetRootElement(doc, root);

            // 小工具：追加一个 <Partition id="..." size="..."/>
            auto add_partition = [root](const char* name, const char* sizeStr) -> bool
            {
                xmlNodePtr pn = xmlNewChild(root, nullptr, BAD_CAST"Partition", nullptr);
                if (!pn) return false;
                if (!xmlNewProp(pn, BAD_CAST"id", BAD_CAST name)) return false;
                if (!xmlNewProp(pn, BAD_CAST"size", BAD_CAST sizeStr)) return false;
                return true;
            };
            ...
            if (!app_->flash.isCMethod)
            {
                if (!io_->part_count)
                {
                    ...
                    xmlFreeDoc(doc);
                    return make_error(FlashErrorCode::PartitionTableNotLoaded,
                ...
                for (int i = 0; i < io_->part_count; ++i)
                {
                    ...
                    else
                    {
                        std::snprintf(sizeStr, sizeof(sizeStr), "%lld",
                                      static_cast<long long>(p.size >> 20));
                    }
                    if (!add_partition(p.name, sizeStr))
                    {
                        build_ok = false;
                        break;
                    }
            ...
            else
            {
                ...
                for (int i = 0; i < c; ++i)
                {
                    const char* name = io_->Cptable[i].name;
                    char sizeStr[32];
                    ...
                    {
                        std::snprintf(sizeStr, sizeof(sizeStr), "%lld",
                                      static_cast<long long>(io_->Cptable[i].size >> 20));
                    }
                    if (!add_partition(name, sizeStr))
                    {
                        build_ok = false;
                        break;
                    }
            ...
            {
                xmlFreeDoc(doc);
                DEG_LOG(E, "exportPartitionTableToXml: failed to build XML nodes");
                return make_error(FlashErrorCode::InternalError, "failed to build XML");
            }

            // ---- 保存到文件（带缩进格式，UTF-8） ----
            int written = xmlSaveFormatFileEnc(output_path.c_str(), doc, "UTF-8", 1);
            xmlFreeDoc(doc);
