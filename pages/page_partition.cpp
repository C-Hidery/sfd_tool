                      ...
                      const std::vector<sfd::DevicePartitionInfo>& partitions)
{
    ...
    for (const auto& part : unmatched_parts)
    {
        ...
        else if (matching_files.size() > 1)
        {
            ...
            DEG_LOG(
                W,
                "[inspect] conflict: partition '%s' matches multiple files (case-insensitive), abandoning partition.",
                part.name.c_str());
                                 ...
                                 const std::vector<sfd::DevicePartitionInfo>& partitions)
{
    ...
    for (const auto& part : unmatched_parts)
    {
        ...
        else if (matching_files.size() > 1)
        {
            ...
            DEG_LOG(
                W,
                "[restore-folder] conflict: partition '%s' matches multiple files (case-insensitive): %s, abandoning partition.",
                part.name.c_str(), [&]()
                {
                    std::string list;
...
void on_button_clicked_list_force_write(GtkWidgetHelper helper)
{
    ...
    bool i_op = showConfirmDialog(parent, _(_(("Confirm"))),
                                  _("Force writing partitions may brick the device, do you want to continue?"));
    if (!i_op) return;
...
void on_button_clicked_export_part_xml(GtkWidgetHelper helper)
{
    ...
    std::string savePath = showSaveFileDialog(GTK_WINDOW(parent), "partition_table.xml",
                                              {{_("XML files (*.xml)"), "*.xml"}});
    if (savePath.empty())
    {
    ...
    std::thread([helper]() mutable
    {
        ...
        gui_idle_call_wait_drag([helper, st, output_dir]() mutable
        {
            if (!st.success)
            {
                ...
                {
                    showErrorDialog(
                        GTK_WINDOW(helper.getWidget("main_window")), _(_(_("Error"))), _("partition backup cancelled"));
                }
                else
                {
...
void confirm_partition_c(GtkWidgetHelper helper)
{
    ...
    bool i_is = showConfirmDialogSyncInThread(
        GTK_WINDOW(helper.getWidget("main_window")),
        _("Confirm"),
        _(
    ...
    if (i_is)
    {
        ...
        else
        {
            showInfoDialogSyncInThread(
                ...
                _("Partition list found in device, compatibility-method mode is disabled."));
            return;
        }
        std::vector<sfd::DevicePartitionInfo> partitions;
            ...
            for (std::size_t idx = 0; idx < total; ++idx)
            {
                ...
                if (!st.success)
                {
                    ...
                    gui_idle_call_wait_drag(
                        [parent, st, part_name = item.part.name, part_size = item.part.size, image_size = item.
                            image_size]() mutable
                        {
