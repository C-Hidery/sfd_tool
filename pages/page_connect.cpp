...
void on_button_clicked_fdl_exec(GtkWidgetHelper helper)
{
    ...
    if (fdl1_loaded > 0)
    {
        ...
        if (g_app_state.device.device_mode != SPRD4)
        {
            ...
            if (!fi)
            {
                ...
                showErrorDialogSyncInThread(
                    GTK_WINDOW(helper.getWidget("main_window")), _("Error"), _("File does not exist."));
                return;
            }
            fi.close();
        ...
        {
            bool result = showConfirmDialogSyncInThread(
                GTK_WINDOW(helper.getWidget("main_window")), _("Confirm"),
                _("Device can be booted without FDL in SPRD4 mode, continue?"));
            if (result)
            {
        ...
        */
        ...
        {
            showInfoDialog(GTK_WINDOW(helper.getWidget("main_window")), _("FDL2 Executed"),
                           _("FDL2 executed successfully!"));
    ...
    else
    {
        ...
        if (fdl_addr == 0x5500 || fdl_addr == 0x65000800)
        {
            ...
            {
                showInfoDialog(
                    GTK_WINDOW(helper.getWidget("main_window")), _("High Speed Mode Enabled"),
                    _("Do not set block size manually in high speed mode!"));
            },GTK_WINDOW(helper.getWidget("main_window")));
        }
        ...
        if (waitFDL1 == -1)
        {
            ...
            {
                showInfoDialog(
                    GTK_WINDOW(helper.getWidget("main_window")), _("FDL1 Executed"), _("FDL1 executed successfully!"));
...
void on_button_clicked_connect(GtkWidgetHelper helper, int argc, char** argv)
{
    ...
    if (argc > 1 && !strcmp(argv[1], "--reconnect"))
    {
        stage = 99;
        showInfoDialogSyncInThread(
            GTK_WINDOW(helper.getWidget("main_window")), _("Tips"),
            _(
    ...
    if (!fdl2_executed)
    {
        ...
        showInfoDialogSyncInThread(
            GTK_WINDOW(helper.getWidget("main_window")), _("Tips"), _("Please execute FDL file to continue!"));
        if (g_app_state.device.device_mode == SPRD4)
        {
            showInfoDialogSyncInThread(
                GTK_WINDOW(helper.getWidget("main_window")), _("Tips"),
            ...
            != SPRD4)
        {
            ...
            i_is = showConfirmDialogSyncInThread(
                GTK_WINDOW(helper.getWidget("main_window")), _("Confirm"),_(
                    "Last FDL path and addr Info detected, do you want to load FDL to FDL2 mode automatically?"));
            if (i_is)
            {
                ...
                if (autoFDL1Suc)
                {
                    ...
                    helper.setEntryText(helper.getWidget("fdl_addr"),
                                        uint32_to_hex_string(static_cast<uint32_t>(std::stoul(cfg.last_fdl2_addr))));
                    DEG_LOG(I, "Loaded FDL info: %s at address: %s", cfg.last_fdl2_path.c_str(),
                            uint32_to_hex_string(static_cast<uint32_t>(std::stoul(cfg.last_fdl2_addr))).c_str());
                    on_button_clicked_fdl_exec(helper);
                }
            }
        }
