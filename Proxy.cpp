...
// ICommChannel 接口（与 BMPlatform.h 保持一致）
class ICommChannel {
public:
...
// 管道命令定义（与 BMPlatform.cpp 中的定义保持一致）
enum ProxyCommand {
...
DWORD HandleCreateChannel() {
    ...
    if (!g_pfCreateChannel(&pChannel, CHANNEL_TYPE_COM)) {
        return 0;
    }
...
void ProcessCommand(const CommandPacket& cmd, ResponsePacket& resp) {
    ...
    switch (cmd.cmd) {
        case CMD_CREATE_CHANNEL: {
            DWORD channelId = HandleCreateChannel();
            resp.result = channelId;
            resp.success = (channelId != 0);
            break;
        }
        ...
        case CMD_FREE_MEM: {
            HandleFreeMem(cmd.objectId, (LPVOID)cmd.param1);
            resp.success = TRUE;
            break;
        }
...
int main() {
    ...
    std::cout << "Waiting for 64-bit client connection..." << std::endl;
    ...
    std::cout << "64-bit client connected" << std::endl;
