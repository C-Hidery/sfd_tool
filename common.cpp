...
void check_root_permission(GtkWidgetHelper helper)
{
    ...
    {
        // not root
        showWarningDialogSyncInThread(
            GTK_WINDOW(helper.getWidget("main_window")), _(_(_("Warning"))), _(
    ...
    if (mode == 1 && len == nread)
        do
        {
            ...
            if (READ32_LE(buf + 0x10) != (int)nread1 - 0x200 || // data size
                ...
                READ32_LE(buf + 0x28) != (int)nread1 + 0x60)
            {
                // sign data offset
...
void w_force_repair_prev(partition_t* ptable, int part_count, int i)
{
    ...
    if (w_force_derive_name(i > 1 ? ptable[i - 2].name : nullptr, ptable[i - 1].name,
                            i < part_count ? ptable[i].name : nullptr, orig, sizeof(orig)))
    {
        bool is_allowed_to_repair = false;
        if (isHelperInit)
        {
            is_allowed_to_repair = showConfirmDialogSyncInThread(
                GTK_WINDOW(helper.getWidget("main_window")), _("Confirm"),
                _("Do you want to repair the leftover w_force partition name?"));
        }
        else
        {
            std::string n;
...
partition_t* partition_list_d(spdio_t* io)
{
    ...
    if (gPartInfo.size)
    {
        ...
        {
            showWarningDialogSyncInThread(
                GTK_WINDOW(helper.getWidget("main_window")), _("Warning"),
                _("w_force partition detected in Compatibility-method mode, please check the partitions!"));
        }
    }
...
#if USE_LIBUSB
    int err = libusb_bulk_transfer(io->dev_handle, io->endp_out, rawbuf, n, &ret, io->timeout);
    //libusb will fail with rawbuf
    ...
    if (!CMethod)
    {
        for (i = 0; i < io->part_count; i++)
        {
            ...
            if (i + 1 == io->part_count)
                WRITE32_LE(buf + 0x48, ~0);
            else
                WRITE32_LE(buf + 0x48, (*(io->ptable + i)).size >> 20);
            buf += 0x4c;
        }
    ...
    else
    {
        for (i = 0; i < io->part_count_c; i++)
        {
            ...
            if (i + 1 == io->part_count_c)
                ...
                WRITE32_LE(buf + 0x48, (*(io->Cptable + i)).size >> 20);
            buf += 0x4c;
        }
...
fallback_to_ansi:
    ...
    for (int i = 0; i < partition_count; i++)
    {
        ...
        else if (selected_ab == 2 && namelen > 2 && 0 == my_stricmp(fn + namelen - 2, "_a"))
        {
            partitions[i].written_flag = 1;
            continue;
        }
...
{
    const char* list[] = {
        "vbmeta", "vbmeta_system", "vbmeta_vendor", "vbmeta_system_ext", "vbmeta_product", "vbmeta_odm", nullptr
    };
    char ch = '\0';
    for (int i = 0; list[i] != nullptr; i++)
    {
        w_mem_to_part_offset(io, list[i], 0x7B, (uint8_t*)&ch, 1, step, CMethod);
    } // DHTB
}
...
int load_partition_unify(spdio_t* io, const char* name, const char* fn, unsigned step, int CMethod)
{
    ...
    strcpy(name0, name);
    if (strlen(name0) >= sizeof(name0) - 4)
    {
    ...
    if (size0 == size1)
    {
        if (!strcmp(name0, "vbmeta"))
        {
            ...
            if (!fi)
            {
                DEG_LOG(E, "fopen %s failed\n", fn);
                return 1;
            }
            if (fi.seek(0x7B, SEEK_SET) != 0)
            {
                DEG_LOG(E, "fseek failed");
                return 1;
            }
            if (fi.write(&ch, 1, 1) != 1)
            {
                DEG_LOG(E, "fwrite failed\n");
                return 1;
            }
        }
