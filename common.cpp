...
bool pac_extract(const char* fn, const char* folder)
{
    ...
    std::string display_name_spl = std::to_string(index) + ". splloader";
    std::string size_str_spl;
    if (spl_size < 1024) size_str_spl = std::to_string(spl_size) + " B";
    else size_str_spl = std::to_string(spl_size / 1024) + " KB";
    gtk_list_store_set(store, &iter_spl, 0, display_name_spl.c_str(), 1, size_str_spl.c_str(), 2, "splloader", -1);
    index++;
    for (const auto& partition : partitions) {
        GtkTreeIter iter;
        gtk_list_store_append(store, &iter);
        std::string dn = std::to_string(index) + ". " + partition.name;
        std::string sz;
        if (partition.size < 1024) sz = std::to_string(partition.size) + " B";
        else if (partition.size < 1024 * 1024) sz = std::to_string(partition.size / 1024) + " KB";
        else if (partition.size < 1024 * 1024 * 1024) sz = std::to_string(partition.size / (1024 * 1024)) + " MB";
        else sz = std::to_string(partition.size / (1024 * 1024 * 1024.0)) + " GB";
        gtk_list_store_set(store, &iter, 0, dn.c_str(), 1, sz.c_str(), 2, partition.name, -1);
        index++;
    }
